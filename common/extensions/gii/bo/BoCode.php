...
class BoCode extends CCodeModel{
    ...
    public function prepare(){
        ...
        foreach($tables as $table)
        {
            ...
            $columns=$this->removeSlash($table->columns);
            $params=array(
                'tableName'=>$schema==='' ? $tableName : $schema.'.'.$tableName,
                'modelClass'=>$className,
                'formClass'=>$this->removeMany($className),
                'columns'=>$columns,
            ...
            $this->files[]=new CCodeFile(
                Yii::getPathOfAlias($this->modelPath).'/'.$this->removeMany($className).'Form.php',
                $this->render($templatePath.'/bo.php', $params)
            );
        }
    }

    public function removeMany($className){
        if($this->endsWith($className, 'ies')){
            return substr($className, 0, -strlen('ies')).'y';
        }elseif($this->endsWith($className, 's')){
            return substr($className, 0, -strlen('s'));
        }else{
            return $className;
        }
    }

    public function removeSlash($arrColumn){
        $result = array();
        foreach($arrColumn as $key=>$col){
            $temp = explode("_", $col->name);
            $str = '';
